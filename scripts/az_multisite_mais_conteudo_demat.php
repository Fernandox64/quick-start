...
/**
 * Mais conteudo de demonstracao pro site DEMAT: mais noticias, mais
...
$dir_n = 'public://imported/demat-noticias';
$dir_a = 'public://imported/demat-avatares';
...
// ---- Mais noticias ----
$mais_noticias = [
  'Departamento inicia ciclo de seminários de matemática pura',
  'Grupo de extensão leva jogos matemáticos a escolas públicas',
  'Convocação para eleição de coordenação do departamento',
  'Resultado do processo seletivo de mestrado é divulgado',
  'Professores participam de congresso internacional de matemática',
  'Departamento firma convênio com centro de pesquisa nacional',
  'Nova disciplina optativa de matemática discreta é aberta',
];
...
foreach ($mais_noticias as $i => $titulo) {
  ...
  az2_gera_banner($fs->realpath($uri), 'DEMAT', $cor, $font_bold);
  ...
  $node = Node::create([
    'type' => 'az_news',
    'title' => $titulo,
    'field_az_summary' => ['value' => 'Notícia de demonstração do site do Departamento de Matemática.', 'format' => 'plain_text'],
...
$mais_professores = [
  ['Paulo', 'Henrique Nogueira', 'Professor Adjunto', 'Matemática Discreta'],
  ['Camila', 'Barros Fontes', 'Professora Titular', 'Equações Diferenciais'],
  ['Diego', 'Correia Vasconcelos', 'Professor Associado', 'Teoria dos Números'],
  ['Isabela', 'Martins Ferreira', 'Professora Adjunta', 'Matemática Financeira'],
];
$p_existentes = 6;
foreach ($mais_professores as $i => [$fname, $lname, $titulo, $area]) {
  ...
  $node = Node::create([
    ...
    'field_az_body' => ['value' => "<p>{$titulo} do Departamento de Matemática, com pesquisa na área de {$area}. Perfil de demonstração (dados fictícios).</p>", 'format' => 'az_standard'],
    'field_az_media_image' => ['target_id' => $media->id()],
...
// ---- Eventos ----
$eventos = [
  ['Semana da Matemática 2026', 10, 9, 12],
  ['Seminário de Topologia', 20, 14, 16],
  ['Defesa de dissertação de mestrado', 27, 10, 11],
  ['Oficina de matemática para calouros', 35, 9, 17],
  ['Colóquio do Departamento de Matemática', 45, 15, 16],
];
...
foreach ($eventos as [$titulo, $dias, $hi, $hf]) {
  ...
  $node = Node::create([
    'type' => 'az_event',
    'title' => $titulo,
    'field_az_summary' => ['value' => 'Evento de demonstração do Departamento de Matemática.', 'format' => 'plain_text'],
