...
/**
 * Conteudo extra de demonstracao pro site DEMAT: gera banners e avatares
...
$dir_noticias = 'public://imported/demat-noticias';
$dir_avatares = 'public://imported/demat-avatares';
...
$noticias = [
  'Departamento de Matemática promove Semana da Matemática 2026',
  'Novo grupo de pesquisa em matemática aplicada',
  'Alunos se destacam em olimpíada de matemática',
  'Editais de monitoria abertos para o próximo semestre',
  'Seminário de topologia reúne pesquisadores',
  'Parceria com universidade estrangeira é firmada',
  'Professor do departamento publica livro de análise numérica',
  'Departamento lança novo curso de extensão',
];
...
foreach ($noticias as $i => $titulo) {
  ...
  az_gera_banner($realpath, 'DEMAT', $cor, $font_bold);
  ...
  $node = Node::create([
    ...
    'title' => $titulo,
    'field_az_summary' => [
      'value' => 'Notícia de demonstração do site do Departamento de Matemática - conteúdo genérico gerado para o teste de multisite.',
      'format' => 'plain_text',
    ],
    'field_az_body' => [
...
$professores = [
  ['André', 'Luiz Batista', 'Professor Associado', 'Álgebra'],
  ['Fernanda', 'Cristina Melo', 'Professora Adjunta', 'Análise Real'],
  ['Marcelo', 'Henrique Dias', 'Professor Titular', 'Topologia'],
  ['Larissa', 'Souza Ramos', 'Professora Associada', 'Matemática Aplicada'],
  ['Rodrigo', 'Teixeira Alves', 'Professor Adjunto', 'Estatística'],
  ['Beatriz', 'Oliveira Santos', 'Professora Associada', 'Geometria Diferencial'],
];
...
foreach ($professores as $i => [$fname, $lname, $titulo, $area]) {
  ...
  $node = Node::create([
    ...
    'field_az_body' => [
      'value' => "<p>{$titulo} do Departamento de Matemática, com pesquisa na área de {$area}. Perfil de demonstração (dados fictícios).</p>",
      'format' => 'az_standard',
    ],
    'field_az_media_image' => ['target_id' => $media->id()],
