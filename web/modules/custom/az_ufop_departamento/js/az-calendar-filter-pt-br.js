/**
 * @file
 * Copia traduzida (pt-br) de az_event/js/az_calendar_filter.js - o widget
 * de datepicker do filtro de eventos tem os botoes "Today/This Week/This
 * Month" e os nomes de mes/dia do jQuery UI Datepicker escritos direto no
 * JS, sem passar por Drupal.t() - o pacote de traducao nunca alcanca isso.
 * Trocada via hook_library_info_alter() no lugar do arquivo original, pra
 * nao editar o profile az_quickstart diretamente. Mesma logica do arquivo
 * original, so com os textos traduzidos.
 */
      ...
      $(once('azCalendarFilter', '.az-calendar-filter-wrapper', context))
        ...
        .each(function () {
          ...
          $buttonWrapper.append(
            '<div class="d-grid gap-2"><button type="button" class="btn btn-outline-blue calendar-filter-button calendar-filter-today">Hoje</button>' +
              '<button type="button" class="btn btn-outline-blue calendar-filter-button calendar-filter-week">Esta Semana</button>' +
              '<button type="button" class="btn btn-outline-blue calendar-filter-button calendar-filter-month mb-2">Este Mês</button></div>',
          );
